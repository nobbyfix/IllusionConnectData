...
function HomeMediator:mailRedPoint()
	local needLevel = ConfigReader:getDataByNameIdAndKey("ConfigValue", "MailBox_UnlockLevel", "content") or 1

	if not self._systemKeeper:isConditionReached({
		LEVEL = tonumber(needLevel)
	}) then
		return false
	end
...
function HomeMediator:walkthroughRedPoint()
	return false
end
