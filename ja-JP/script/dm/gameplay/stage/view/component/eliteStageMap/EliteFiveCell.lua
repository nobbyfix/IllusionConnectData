EliteFiveCell = class("EliteFiveCell", ChapterBase)
local kBtnHandlers = {}

function EliteFiveCell:initialize(data)
...
function EliteFiveCell:update(stageType, redPointTab)
	...
	self:initChapterPanel(self._chapter1, "E08", callFunc)
	...
	self:initChapterPanel(self._chapter2, "E09", callFunc)
...
function EliteFiveCell:initView()
	self:initWidget()
	self:initAnim()
	self:adjustUI()
end

function EliteFiveCell:onScroll(offX)
end

function EliteFiveCell:initWidget()
...
function EliteFiveCell:adjustUI()
