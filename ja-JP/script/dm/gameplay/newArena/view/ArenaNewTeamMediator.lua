...
ArenaNewTeamMediator = class("ArenaNewTeamMediator", CommonTeamMediator, _M)

ArenaNewTeamMediator:has("_stageSystem", {
	is = "r"
}):injectWith("StageSystem")
ArenaNewTeamMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")
ArenaNewTeamMediator:has("_systemKeeper", {
	is = "r"
}):injectWith("SystemKeeper")
ArenaNewTeamMediator:has("_arenaNewSystem", {
	is = "r"
}):injectWith("ArenaNewSystem")
ArenaNewTeamMediator:has("_gameServerAgent", {
	is = "r"
...
function ArenaNewTeamMediator:onRegister()
...
function ArenaNewTeamMediator:enterWithData(data)
	data = data or {}
	self._data = data
...
function ArenaNewTeamMediator:dispose()
	super.dispose(self)
end

function ArenaNewTeamMediator:resumeWithData()
	self._canClick = true
...
function ArenaNewTeamMediator:initData(team)
...
function ArenaNewTeamMediator:updateData()
	self._petList = self._stageSystem:getNotOnTeamPet(self._teamPets)
	self._petListAll = {}
...
function ArenaNewTeamMediator:removeExceptHeros()
...
function ArenaNewTeamMediator:showOneKeyHeros()
...
function ArenaNewTeamMediator:setupView()
	self:initLockIcons()
	self:refreshPetNode()
...
function ArenaNewTeamMediator:initWidgetInfo()
	...
	self._panelTime = self._main:getChildByFullName("Panel_time")

	self._panelTime:getChildByFullName("Image_8"):addClickEventListener(function ()
		self:clickRivalTeam()
	end)
...
function ArenaNewTeamMediator:ignoreSafeArea()
...
function ArenaNewTeamMediator:createTeamCell(cell, index)
...
function ArenaNewTeamMediator:setLabelEffect()
...
function ArenaNewTeamMediator:createCardsGroupName()
...
function ArenaNewTeamMediator:onClickCell(sender, eventType, index)
...
function ArenaNewTeamMediator:changeOwnPet(cell)
	local id = cell.id
...
function ArenaNewTeamMediator:insertTeamPet(cell, isDrag)
	local id = cell.id
	local animId = nil
	local index = table.indexof(self._petListAll, id)
...
function ArenaNewTeamMediator:createMovingPet(cell, type)
	local petNode = nil
...
function ArenaNewTeamMediator:sortOnTeamPets()
	self._heroSystem:sortOnTeamPets(self._teamPets)
end

function ArenaNewTeamMediator:onClickOnTeamPet(sender, eventType)
	if eventType == ccui.TouchEventType.began then
		self._isOnOwn = false
	elseif eventType == ccui.TouchEventType.moved then
		local beganPos = sender:getTouchBeganPosition()
...
function ArenaNewTeamMediator:changeTeamPet(index, endPos)
...
function ArenaNewTeamMediator:removeTeamPet(index)
	AudioEngine:getInstance():playEffect("Se_Click_Putdown", false)
...
function ArenaNewTeamMediator:refreshViewBySort()
	self:updateData()
	self:refreshMasterInfo()
	self:refreshListView()
	self:refreshPetNode()
...
function ArenaNewTeamMediator:refreshView()
	self:initData()
	self:refreshMasterInfo()
	self:refreshListView()
	self:refreshPetNode()
...
function ArenaNewTeamMediator:refreshListView(ignoreAdjustOffset)
...
function ArenaNewTeamMediator:initHero(node, info)
...
function ArenaNewTeamMediator:refreshPetNode()
...
function ArenaNewTeamMediator:initTeamHero(node, info)
	local heroId = info.id
...
function ArenaNewTeamMediator:refreshCombatAndCost()
...
function ArenaNewTeamMediator:changeMasterId(event)
	self._oldMasterId = self._curMasterId
...
function ArenaNewTeamMediator:refreshMasterInfo()
	local masterData = self._masterSystem:getMasterById(self._curMasterId)
...
function ArenaNewTeamMediator:refreshButtons()
	self._teamBreakBtn:setVisible(false)
	self._teamOneKeyBtn:setVisible(false)
end

function ArenaNewTeamMediator:checkButtonVisible()
...
function ArenaNewTeamMediator:checkToExit()
...
function ArenaNewTeamMediator:hasChangeTeam()
...
function ArenaNewTeamMediator:onClickFight(sender, eventType)
	if #self._teamPets < 1 then
		self:dispatch(ShowTipEvent({
			duration = 0.2,
...
function ArenaNewTeamMediator:onEnterBattle()
	if not self._arenaNewSystem:getCurSeasonData() then
		self:dispatch(ShowTipEvent({
	...
	self._arenaNewSystem:setShowViewAfterBattle("ArenaNewView")
	self._arenaNewSystem:requestBeginBattle(self._rivalTeamIndex, callback)
end

function ArenaNewTeamMediator:onClickRival()
	AudioEngine:getInstance():playEffect("Se_Click_Battle", false)

	local index = self._rivalTeamIndex
...
function ArenaNewTeamMediator:showMatchOffset()
	local data = {
	...
	local delegate = {
		willClose = function (self, popupMediator, data)
			if data.response == "ok" then
				outSelf._arenaNewSystem:requestGainChessArena(nil, true)
				outSelf:dismiss()
			end
		end
	}
...
function ArenaNewTeamMediator:showDiffRankAlert()
	local data = {
		title = Strings:get("Tip_Remind"),
		title1 = Strings:get("UITitle_EN_Tishi"),
		content = Strings:get("ClassArena_UI76"),
		sureBtn = {}
	}
	local outSelf = self
	local delegate = {
...
function ArenaNewTeamMediator:sendSpChangeTeam(callBack)
	local sendData = {}
	local hasHero = false
...
function ArenaNewTeamMediator:onClickRule()
...
function ArenaNewTeamMediator:onClickOneKeyBreak()
...
function ArenaNewTeamMediator:onClickSaveTeam()
	local function func()
		self._tempTeams = {}
		self._teamPets = self._developSystem:getSpTeamByType(self._stageType):getHeroes()
...
function ArenaNewTeamMediator:onClickBack()
	local function func()
		self:dismiss()
	end
...
function ArenaNewTeamMediator:changeTeamByMode(event)
...
function ArenaNewTeamMediator:clickRivalTeam()
	...
	local function callback()
		local view = self:getInjector():getInstance("ArenaNewTeamListView")
		local event = ViewEvent:new(EVT_PUSH_VIEW, view, nil, {
			index = self._rivalTeamIndex,
			anim = self._anim,
			myHeroIds = self._teamPets
		})
...
function ArenaNewTeamMediator:runTransLateAction(callback)
