...
function ExplorePointTaskMediator:createTeamCell(cell, index)
	...
	elseif data.type == "task" then
		...
		desc:setString(taskDesc .. "（" .. currentValue .. "/" .. targetValue .. "）")
		panel:getChildByName("num"):setString(Strings:get("EXPLORE_UI19", {
			num = data:getDp()
		}))
		...
		if showTypeName and #showTypeName > 0 then
			local posX = desc:getPositionX() + desc:getContentSize().width + 8
			...
			for i = 1, #showTypeName do
				...
				image:addTo(panel)
				image:setPosition(cc.p(posX, 12))
