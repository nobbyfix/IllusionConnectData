...
function RecruitMainMediator:initView()
	...
	local title1 = cc.Label:createWithTTF(Strings:get("Story_Skip"), TTF_FONT_FZYH_R, 20)
...
function RecruitMainMediator:initCostNode(recruitCost, costNode, offCount)
	...
	if costCount == 0 then
		...
		if costId == "IM_DiamondDraw" and not self._bagSystem:checkCostEnough(costId, 1) then
			...
			local price = costCount == 1 and DrawCard_SinglePrice or DrawCard_TenTimesPrice
			costCount = costCount * price

			name1:setPositionY(14)
		end
...
function RecruitMainMediator:refreshMiddleTime()
	...
	self._middleTimeNode:getChildByFullName("count"):setPositionX(-20)
...
function RecruitMainMediator:updateView()
	...
	if hasLeft ~= "" then
		...
		self._leftCountNode:getChildByFullName("text"):setPositionX(240)
...
function RecruitMainMediator:refreshUREQuipInfo()
	...
	if equipInfo and type == RecruitPoolType.kActivityUREquip then
		...
		for i = 1, #equipInfo do
			...
			local config = ConfigReader:getRecordById("Compose", equipId)
			local iconPanel = equipNode:getChildByName("icon")

			iconPanel:removeAllChildren()

			local icon = IconFactory:createPic(config.Item1)

			icon:addTo(iconPanel):center(iconPanel:getContentSize())
			IconFactory:bindTouchHander(iconPanel, IconTouchHandler:new(self), {
				type = config.Item1.type,
				code = config.Item1.id,
				amount = config.Item1.amount
			}, {
				swallowTouches = true,
				needDelay = true
			})
