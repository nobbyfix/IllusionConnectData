...
function ccDrawSolidRect(origin, destination, color)
	local vertices = {
		origin,
		CCPoint:__call(destination.x, origin.y),
		destination,
		CCPoint:__call(origin.x, destination.y)
	}
...
function ccDrawCircle(center, radius, angle, segments, drawLineToCenter)
	ccDrawCircleScale(center, radius, angle, segments, drawLineToCenter, 1, 1)
end
...
function ccDrawQuadBezier(origin, control, destination, segments)
	...
	local function initBuffer()
		...
		for i = 1, segments do
			vertices[2 * i - 1] = math.pow(1 - t, 2) * origin.x + 2 * (1 - t) * t * control.x + t * t * destination.x
			vertices[2 * i] = math.pow(1 - t, 2) * origin.y + 2 * (1 - t) * t * control.y + t * t * destination.y
			t = t + 1 / segments
		end
...
function ccDrawCubicBezier(origin, control1, control2, destination, segments)
	...
	local function initBuffer()
		...
		for i = 1, segments do
			vertices[2 * i - 1] = math.pow(1 - t, 3) * origin.x + 3 * math.pow(1 - t, 2) * t * control1.x + 3 * (1 - t) * t * t * control2.x + t * t * t * destination.x
			vertices[2 * i] = math.pow(1 - t, 3) * origin.y + 3 * math.pow(1 - t, 2) * t * control1.y + 3 * (1 - t) * t * t * control2.y + t * t * t * destination.y
			t = t + 1 / segments
		end
