...
module("storysandbox.story10_1a")
setmetatable(_M, {
	__index = __story_sandbox__
})

local scene_story10_1a = {
	actions = {}
}
scenes.scene_story10_1a = scene_story10_1a

function scene_story10_1a:stage(args)
	return {
		name = "root",
		type = "Stage",
		id = "root",
...
function scene_story10_1a.actions.start_story10_1a(_root, args)
	return sequential({
		concurrent({
			act({
				action = "show",
...
function scene_story10_1a.actions.start10_1a1(_root, args)
	return sequential({
		concurrent({
			act({
				action = "fadeOut",
...
function scene_story10_1a.actions.start10_1a2(_root, args)
	return sequential({
		concurrent({
			act({
				action = "fadeOut",
...
function scene_story10_1a.actions.start10_1a3(_root, args)
	return sequential({
		act({
			action = "changeTexture",
			actor = __getnode__(_root, "CLMan_face"),
		...
		concurrent({
			act({
				action = "play",
				actor = __getnode__(_root, "Se_Story_Knock")
			}),
			act({
				action = "speak",
				actor = __getnode__(_root, "dialogue"),
		...
		act({
			action = "fadeIn",
			actor = __getnode__(_root, "BLTu2_speak"),
			args = function (_ctx)
				return {
					duration = 0.1
				}
...
local function story10_1a(args)
	return sequential({
		enterScene({
			args = function (_ctx)
				return {
					action = "start_story10_1a",
					scene = scene_story10_1a
				}
			end
		})
	})
