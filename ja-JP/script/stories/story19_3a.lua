...
module("storysandbox.story19_3a")
setmetatable(_M, {
	__index = __story_sandbox__
})

local scene_story19_3a = {
	actions = {}
}
scenes.scene_story19_3a = scene_story19_3a

function scene_story19_3a:stage(args)
	return {
		name = "root",
		type = "Stage",
		id = "root",
...
function scene_story19_3a.actions.start_story19_3a(_root, args)
	return sequential({
		act({
			action = "activateNode",
		...
		concurrent({
			act({
				action = "updateNode",
				actor = __getnode__(_root, "BLTu_speak1"),
				args = function (_ctx)
					return {
						opacity = 0
					}
...
local function story19_3a(args)
	return sequential({
		enterScene({
			args = function (_ctx)
				return {
					action = "start_story19_3a",
					scene = scene_story19_3a
				}
			end
		})
	})
