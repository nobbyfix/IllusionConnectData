...
function ccs.sendTriggerEvent(event)
	...
	for i = 1, table.getn(triggerObjArr) do
		...
		if triObj ~= nil and triObj:detect() then
			triObj:done()
		end
	end
end
...
ccs.ObjectFactory = cclass("ObjectFactory")
...
ccs.TriggerObj = cclass("TriggerObj")
...
function ccs.TriggerObj:detect()
	...
	for i = 1, table.getn(self._cons) do
		...
		if obj ~= nil and obj.detect ~= nil then
	...
	if cons ~= nil then
		...
		for i = 1, count do
			...
			if className ~= nil then
				...
				assert(obj ~= nil, string.format("cclass named %s can not implement!", className))
	...
	if actions ~= nil then
		...
		for i = 1, count do
			...
			if className ~= nil then
				...
				assert(act ~= nil, string.format("cclass named %s can not implement!", className))
...
ccs.TriggerMng = cclass("TriggerMng")
...
function ccs.TriggerMng:remove(event, obj)
	...
	if self._eventTriggers == nil then
		return false
	end

	local triObjects = self._eventTriggers[event]

	if triObjects == nil then
		return false
	end
...
function ccs.TriggerMng:removeObjByEvent(event, obj)
	...
	for i = 1, table.getn(triObjects) do
		...
		if triObject ~= nil and triObject == obj then
			triObject:remvoeAll()
...
function ccs.TriggerMng:isEmpty()
	return self._eventTriggers ~= nil or table.getn(self._eventTriggers) <= 0
end
...
function ccs.AnimationInfo(_name, _startIndex, _endIndex)
	assert(_name ~= nil and type(_name) == "string" and _startIndex ~= nil and type(_startIndex) == "number" and _endIndex ~= nil and type(_endIndex) == "number", "ccs.AnimationInfo() - invalid input parameters")
