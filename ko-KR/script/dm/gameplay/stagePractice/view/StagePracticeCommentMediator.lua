...
function StagePracticeCommentMediator:onClickSendComment(sender, eventType)
	if eventType == ccui.TouchEventType.ended then
		local commentInfo = self._commentInfo:getText()

		if commentInfo == "" or commentInfo == Strings:get("StagePtc_Comment_WordLimit") then
			self:dispatch(ShowTipEvent({
				duration = 0.2,
		...
		if not StringChecker.checkKoreaStr(commentInfo) then
			self:getEventDispatcher():dispatchEvent(ShowTipEvent({
				tip = Strings:get("Common_Tip1")
			}))
		...
		self._stagePracticeSystem:requestPublishPointComment(self._pointId, self._mapId, commentInfo, function (response)
