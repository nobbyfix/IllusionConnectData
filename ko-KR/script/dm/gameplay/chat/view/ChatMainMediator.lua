...
function ChatMainMediator:setUpView()
	...
	local chatTable = self:getView():getChildByFullName("main.chatTable")
	local warningstr = Strings:get("chat_warnning_tip")
	local warnningTip = cc.Label:createWithTTF(warningstr, CUSTOM_TTF_FONT_1, 14)

	warnningTip:enableOutline(cc.c4b(0, 0, 0, 200), 1)
	warnningTip:setAnchorPoint(cc.p(0, 1))
	warnningTip:addTo(chatTable):offset(20, 430)
end
