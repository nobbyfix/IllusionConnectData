...
function ChatOperatorWidget:bindWidgets()
	...
	self._btn_send = bindWidget(self, "main_panel.btn_send", OneLevelViceButton, {
		handler = {
...
function ChatOperatorWidget:enterWithData(data)
	...
	self._editBox:setPlaceholderFontColor(cc.c4b(180, 180, 180, 255))
...
function ChatOperatorWidget:onClickSend(sender, eventType)
	...
	if unlock then
		...
		if self._chatColdTimer then
			...
			local buttonName = Strings:get("Friend_UI10")
			local buttonName1 = Strings:get("UITitle_EN_Fasong")
		...
		elseif not StringChecker.checkKoreaStr(self._content) then
			AudioEngine:getInstance():playEffect("Se_Alert_Error", false)
			self:getEventDispatcher():dispatchEvent(ShowTipEvent({
				tip = Strings:get("Common_Tip1")
			}))
