...
function SettingSystem:getWeatherData()
	...
	if playerArea == "" then
		playerArea = ConfigReader:getDataByNameIdAndKey("ConfigValue", "Default_Weather", "content")
	end
...
function SettingSystem:requestUnBindAccount(accountId, callback)
	local params = {
	...
	if url then
		self._settingService:requestUnBindAccount(url, params, callback)
	else
		self:dispatch(ShowTipEvent({
			tip = "GlobalServer is  nil"
		}))
	end
end
