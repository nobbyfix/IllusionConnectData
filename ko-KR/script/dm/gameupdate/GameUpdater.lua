	...
	self._assetsManager:installAssetsPackage(pkgPaths, 50, true, function (event, details)
		...
		elseif event == "completed" then
			...
			if installVersionsIndex >= 1000 or tonumber(#installVersionsInfo) == tonumber(installVersionsIndex) then
				...
				if PlatformHelper and PlatformHelper:getSdkSource() ~= "" then
					PlatformHelper:postCYData({
						eventKey = "10004"
					})
				end
			...
			if errorCode == 200 then
				if httpData and httpData.ret then
					if httpData.data then
						...
						local supportHttpsVersion = 6113
						local baseVersion = tonumber(app.pkgConfig.packJobId)

						if type(baseVersion) == "number" and supportHttpsVersion < baseVersion then
							for index, cdnUrl in pairs(_G.CDN_URLS) do
								_G.CDN_URLS[index] = string.gsub(cdnUrl, "http:", "https:", 1)
							end
						end
