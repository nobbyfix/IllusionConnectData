...
module("storysandbox.story10_2a")
setmetatable(_M, {
	__index = __story_sandbox__
})

local scene_story10_2a = {
	actions = {}
}
scenes.scene_story10_2a = scene_story10_2a

function scene_story10_2a:stage(args)
	return {
		name = "root",
		type = "Stage",
		id = "root",
...
function scene_story10_2a.actions.start_story10_2a(_root, args)
	return sequential({
		act({
			action = "show",
		...
		concurrent({
			act({
				action = "updateNode",
				actor = __getnode__(_root, "SDTZi_speak_Daily_Daily"),
				args = function (_ctx)
					return {
						opacity = 0
					}
...
local function story10_2a(args)
	return sequential({
		enterScene({
			args = function (_ctx)
				return {
					action = "start_story10_2a",
					scene = scene_story10_2a
				}
			end
		})
	})
