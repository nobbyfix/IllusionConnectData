...
function ShopPackageMainMediator:setTimer(panel, data)
	...
	if remoteTimestamp < endMills and not self._timer[data:getId()] then
		local function checkTimeFunc()
			...
			local image2 = limitePanel:getChildByName("Image_2")
			local times = limitePanel:getChildByName("times")

			times:setString(str)
			limitePanel:setVisible(true)
			image2:setContentSize(cc.size(times:getContentSize().width + 35, image2:getContentSize().height))
			times:setPositionX(image2:getPositionX() - image2:getContentSize().width + 22)
		end
