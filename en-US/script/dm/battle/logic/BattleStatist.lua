...
function SimpleBattleStatist:summarize()
	...
	if GameConfigs and GameConfigs.StatistSetBox then
		self:showDebug({
			players = players,
			totalTime = totalTime,
...
function SimpleBattleStatist:showDebug(data)
	...
	local friend = data.players[playerA:getId()]
	local enemy = data.players[playerB:getId()]
	local director = cc.Director:getInstance()
	local scene = director:getRunningScene()
	local layer = cc.LayerGradient:create(cc.c4b(0, 0, 0, 255), cc.c4b(0, 0, 255, 100))
	...
	local function addTxt(root, pos, txtStr)
		local txt = cc.Label:createWithTTF(txtStr, TTF_FONT_FZYH_M, 20)

		root:addChild(txt, 1000)
		txt:setPosition(pos)
		txt:setColor(cc.c4b(255, 0, 0, 255))
		txt:setAnchorPoint(cc.p(0, 1))
	end

	local deadth = 0
	local all_enemy_unit = {}
	local all_enemy_summon = {}
	local all_enemy_summon_key = {}

	for k, v in pairs(enemy.unitSummary) do
		for k_, v_ in pairs(v.summoned) do
			all_enemy_summon_key[v_] = k
		end
	end

	local death_unit = 0
	local death_summon = 0

	for k, v in pairs(enemy.unitSummary) do
		if all_enemy_summon_key[k] then
			all_enemy_summon[#all_enemy_summon + 1] = v

			if v.dead then
				death_summon = death_summon + 1
			end
		else
			all_enemy_unit[#all_enemy_summon + 1] = v

			if v.dead then
				death_unit = death_unit + 1
			end
		end
	end

	local all_enemy_unit_my = {}
	local all_enemy_summon_my = {}
	local all_enemy_summon_my_key = {}
	local death_unit_my = 0
	local death_summon_my = 0
	local live_summon_my = 0
	local live_unit_my = 0
	local damage = 0
	local cure = 0

	for k, v in pairs(friend.unitSummary) do
		for k_, v_ in pairs(v.summoned) do
			all_enemy_summon_my_key[v_] = k
		end
	...
	for k, v in pairs(friend.unitSummary) do
		if all_enemy_summon_my_key[k] then
			all_enemy_summon_my[#all_enemy_summon_my + 1] = v

			if v.dead then
				death_summon_my = death_summon_my + 1
			else
				live_summon_my = live_summon_my + 1
			end
		else
			all_enemy_unit_my[#all_enemy_unit_my + 1] = v

			if v.dead then
				death_unit_my = death_unit_my + 1
			else
				live_unit_my = live_unit_my + 1
			end
		end
	end

	friend.usedHeroCards = friend.usedHeroCards or {}

	addTxt(layer, cc.p(100, viewsize.height - 80), "战斗统计debug")
	addTxt(layer, cc.p(100, viewsize.height - 150), "消灭伙伴数:" .. death_unit)
	addTxt(layer, cc.p(100, viewsize.height - 200), "消灭召唤物数:" .. death_summon)
	addTxt(layer, cc.p(100, viewsize.height - 250), "我方死亡伙伴数:" .. death_unit_my)
	addTxt(layer, cc.p(100, viewsize.height - 300), "我方死亡召唤物数:" .. death_summon_my)
	addTxt(layer, cc.p(100, viewsize.height - 350), "我方存活伙伴数:" .. live_unit_my)
	addTxt(layer, cc.p(100, viewsize.height - 400), "我方存活召唤物数:" .. live_summon_my)
	addTxt(layer, cc.p(100, viewsize.height - 450), "上阵角色数:" .. #friend.usedHeroCards)
	addTxt(layer, cc.p(100, viewsize.height - 500), "总伤害量:" .. damage)
	addTxt(layer, cc.p(100, viewsize.height - 550), "总恢复量:" .. cure)
end
