ActivityBlockGoldEggMediator = class("ActivityBlockGoldEggMediator", DmAreaViewMediator, _M)

ActivityBlockGoldEggMediator:has("_activitySystem", {
	is = "r"
}):injectWith("ActivitySystem")
ActivityBlockGoldEggMediator:has("_developSystem", {
	is = "r"
...
function ActivityBlockGoldEggMediator:initialize()
	super.initialize(self)
end

function ActivityBlockGoldEggMediator:dispose()
...
function ActivityBlockGoldEggMediator:onRegister()
	...
	self._tipBtn = self._main:getChildByName("tipBtn")

	self._tipBtn:setPositionX(350)
...
function ActivityBlockGoldEggMediator:doReset()
...
function ActivityBlockGoldEggMediator:setupTopInfoWidget()
...
function ActivityBlockGoldEggMediator:enterWithData()
	self._activityId = ActivityId.kActivityEgg
...
function ActivityBlockGoldEggMediator:refreshView()
	self:initData()
	self:updateEggNodes()
end

function ActivityBlockGoldEggMediator:initData(isInit)
	if isInit then
...
function ActivityBlockGoldEggMediator:initView()
...
function ActivityBlockGoldEggMediator:initRewardView()
...
function ActivityBlockGoldEggMediator:createReward(rewardId, pos, got)
	local reward = ConfigReader:getDataByNameIdAndKey("Reward", rewardId, "Content")[1]
	local icon = IconFactory:createRewardIcon(reward, {
		isWidget = true
	})
...
function ActivityBlockGoldEggMediator:refreshViewByRefresh()
...
function ActivityBlockGoldEggMediator:refreshViewByAutoRefresh()
	for i = 1, #self._eggNodes do
...
function ActivityBlockGoldEggMediator:refreshViewNormal()
	for i = 1, #self._eggNodes do
		local image = self._eggList[i].image
		local rewardId = self._eggList[i].rewardId
		local node = self._eggNodes[i]
...
function ActivityBlockGoldEggMediator:createAnimBigReward(parent)
...
function ActivityBlockGoldEggMediator:createAnimNormalReward(parent)
...
function ActivityBlockGoldEggMediator:createAnimNormalEgg(parent)
...
function ActivityBlockGoldEggMediator:updateEggNodes()
...
function ActivityBlockGoldEggMediator:createEggTimer()
	if self._eggTimer then
		self._eggTimer:stop()
...
function ActivityBlockGoldEggMediator:updateRound()
...
function ActivityBlockGoldEggMediator:updateRefreshNum()
...
function ActivityBlockGoldEggMediator:onEggSucc(eggId, response)
	self:refreshView()
	self:onGetRewardCallback(eggId, response)
end

function ActivityBlockGoldEggMediator:onGetRewardCallback(eggId, response)
...
function ActivityBlockGoldEggMediator:onClickEgg(index)
	for itemId, num in pairs(self._costData) do
		local amount = self._bagSystem:getItemCount(itemId)
...
function ActivityBlockGoldEggMediator:onClickOpen5Eggs()
...
function ActivityBlockGoldEggMediator:onClickReward()
	self._activitySystem:showEasterRewards(self._activityId)
end

function ActivityBlockGoldEggMediator:onClickRule()
	local rules = self._eggActivity:getActivityConfig().RuleDesc

	self._activitySystem:showActivityRules(rules)
end

function ActivityBlockGoldEggMediator:onClickRefresh()
	local function func()
		self._refreshing = true
...
function ActivityBlockGoldEggMediator:onClickBack(sender, eventType)
	self:dismiss()
end
