...
local accChargeInfo = ConfigReader:getDataByNameIdAndKey("ConfigValue", "Player_Charge_Activity", "content")
...
local function helpStopSoundAction(soundId)
	if soundId then
...
function HomeMediator:playClimate2Audio()
	...
	if (GameConfigs.closeGuide or not guideAgent:isGuiding() and level > 1) and self._climateEffect == nil and Climate2AudioEffect[self._climateDay] then
...
function HomeMediator:willStartResumeTransition()
	...
	self._rightFuncLayout:getChildByFullName("goldEggsPanel"):setOpacity(0)
...
function HomeMediator:didFinishCoverTransition()
	...
	helpStopSoundAction(self._climateEffect)
	helpStopSoundAction(ResidentSoundID)
...
function HomeMediator:initHomeView()
	...
	self._homeViewBgId = settingSys:getHomeBgId()
...
function HomeMediator:initAnim()
	...
	mainMovieClip:addCallbackAtFrame(17, function ()
		...
		self._rightFuncLayout:getChildByFullName("goldEggsPanel"):runAction(cc.FadeIn:create(0.21))
	end)
	self._menuStateBtn:setOpacity(0)
			...
			touchPanel:addTouchEventListener(function (sender, eventType)
				if eventType == ccui.TouchEventType.ended then
					...
					for _, v in ipairs(_info.voice) do
						local isUnlock = self:checkSoundUnlock(self._showHeroId, v)
					...
					if self._touchTimes > 5 and self._lastClickSound ~= "Voice_" .. self._showHeroId .. "_60" then
						soundTab[#soundTab + 1] = "Voice_" .. self._showHeroId .. "_60"
						weighting = 1.6
					end

					local randomCount = math.ceil(#soundTab * weighting)
					local soundId = soundTab[math.random(1, randomCount)]
					soundId = soundId or "Voice_" .. self._showHeroId .. "_60"
...
function HomeMediator:registerHomeViewEvent()
	...
	schedule(self:getView(), function ()
		local soundId = "Voice_" .. self._showHeroId .. "_05"
		local talkPanel = self._showHeroPanel:getChildByName("talkPanel")
...
function HomeMediator:onGalleryBtn(sender, type)
	if type == ccui.TouchEventType.ended then
		if SDKHelper and SDKHelper:isEnableSdk() then
			SDKHelper:adjustEventTracking(AdjustEventList.ADJUST_TOUCH_HOME_PHOTO_EVENT)
		end
...
function HomeMediator:onChallengeBtn(sender, type)
	if type == ccui.TouchEventType.ended then
		...
		if SDKHelper and SDKHelper:isEnableSdk() then
			SDKHelper:adjustEventTracking(AdjustEventList.ADJUST_TOUCH_BATTLE_DAILY_EVENT)
		end
...
function HomeMediator:checkExtraRedPoint()
	...
	local carnivalActivity = self._activitySystem:getActivityByType("Carnival")
	local carnBtn = self:getView():getChildByFullName("extraActBtn.btn_1")

	if carnivalActivity then
		local realId = carnivalActivity:getId()
		local btnImageName = carnivalActivity:getActivityConfig().Icon

		carnBtn:loadTextureNormal(btnImageName .. ".png", ccui.TextureResType.plistType)
		carnBtn:loadTexturePressed(btnImageName .. ".png", ccui.TextureResType.plistType)
		carnBtn:setVisible(self._activitySystem:checkConditionWithId(realId))

		local carnBtnRedPoint = carnBtn:getChildByName("redPoint")

		carnBtnRedPoint:setVisible(self._activitySystem:hasRedPointForActivity(realId))
	else
		carnBtn:setVisible(false)
	end

	local chargeBtn = self:getView():getChildByFullName("extraActBtn.btn_2")

	if not carnBtn:isVisible() then
		chargeBtn:setPosition(cc.p(carnBtn:getPositionX(), carnBtn:getPositionY()))
	else
