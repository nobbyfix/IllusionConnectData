EliteFourCell = class("EliteFourCell", ChapterBase)
local kBtnHandlers = {}
local onPlayAnim = nil

function EliteFourCell:initialize(data)
...
function EliteFourCell:update(stageType, redPointTab)
	...
	self:initChapterPanel(self._chapter1, "E06", callFunc)
	...
	self:initChapterPanel(self._chapter2, "E07", callFunc)
...
function EliteFourCell:initView()
	self:initWidget()
	self:initAnim()
end

function EliteFourCell:initWidget()
...
function EliteFourCell:onScroll(offX)
...
function EliteFourCell:refreshAnim(isPlaying)
	if isPlaying then
...
function EliteFourCell:initAnim()
