...
module("storysandbox.eventstory_freud_02a")
setmetatable(_M, {
	__index = __story_sandbox__
})

local scene_eventstory_freud_02a = {
	actions = {}
}
scenes.scene_eventstory_freud_02a = scene_eventstory_freud_02a

function scene_eventstory_freud_02a:stage(args)
	return {
		name = "root",
		type = "Stage",
		id = "root",
...
function scene_eventstory_freud_02a.actions.start_eventstory_freud_02a(_root, args)
	return sequential({
		act({
			action = "fadeIn",
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Mus_anjing"),
			args = function (_ctx)
				return {
					isLoop = true
				}
		...
		act({
			action = "activateNode",
			actor = __getnode__(_root, "bg_shu")
		}),
		act({
			action = "fadeOut",
			actor = __getnode__(_root, "curtain"),
		...
		concurrent({
			act({
				action = "play",
				actor = __getnode__(_root, "xiao_jiaoyi"),
				args = function (_ctx)
					return {
						time = -1
					}
		...
		concurrent({
			...
			act({
				...
				args = function (_ctx)
					return {
						name = "freud_dialog_speak_name_14",
						dialogImage = "jq_dialogue_bg_1.png",
						location = "left",
						pathType = "STORY_ROOT",
						speakings = {
		...
		act({
			action = "fadeIn",
			actor = __getnode__(_root, "cg_attack"),
			args = function (_ctx)
				return {
					duration = 1
				}
...
local function eventstory_freud_02a(args)
	return sequential({
		enterScene({
			args = function (_ctx)
				return {
					action = "start_eventstory_freud_02a",
					scene = scene_eventstory_freud_02a
				}
			end
		})
	})
...
stories.eventstory_freud_02a = eventstory_freud_02a
