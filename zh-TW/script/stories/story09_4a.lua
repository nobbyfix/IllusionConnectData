...
module("storysandbox.story09_4a")
setmetatable(_M, {
	__index = __story_sandbox__
})

local scene_story09_4a = {
	actions = {}
}
scenes.scene_story09_4a = scene_story09_4a

function scene_story09_4a:stage(args)
	return {
		name = "root",
		type = "Stage",
		id = "root",
...
function scene_story09_4a.actions.start_story09_4a(_root, args)
	return sequential({
		act({
			action = "show",
		...
		act({
			action = "play",
			actor = __getnode__(_root, "liqi_EXhit")
		}),
		act({
			action = "rock",
			actor = __getnode__(_root, "SGHQShou2_speak"),
		...
		concurrent({
			...
			act({
				action = "play",
				actor = __getnode__(_root, "dunqi_hitold")
			}),
			act({
				action = "moveTo",
				actor = __getnode__(_root, "ZTXChang_blackspeak"),
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Se_Skill_Heal_5")
		}),
		act({
			action = "flashScreen",
			actor = __getnode__(_root, "flashMask"),
		...
		concurrent({
			act({
				action = "play",
				actor = __getnode__(_root, "Se_Skill_Heal_6")
			}),
			act({
				action = "play",
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Se_Story_Gunshot")
		}),
		act({
			action = "play",
			actor = __getnode__(_root, "baozha_EXhit")
		}),
		sleep({
			args = function (_ctx)
				return {
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Se_Skill_Laser_7")
		}),
		act({
			action = "play",
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Se_Skill_Spare")
		}),
		act({
			action = "rotateBy",
			actor = __getnode__(_root, "liqi_EXhit"),
		...
		act({
			action = "play",
			actor = __getnode__(_root, "Mus_Story_Mystery_2"),
			args = function (_ctx)
				return {
					isLoop = false
				}
		...
		act({
			action = "fadeIn",
			actor = __getnode__(_root, "ZTXChang_face2"),
			args = function (_ctx)
				return {
					duration = 0
				}
...
local function story09_4a(args)
	return sequential({
		enterScene({
			args = function (_ctx)
				return {
					action = "start_story09_4a",
					scene = scene_story09_4a
				}
			end
		})
	})
